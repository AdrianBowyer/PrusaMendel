...
// Modified by Adrian to remove bridges and to add a conical cavity for the bottom nut.  This prints more reliably and cleanly.  I
// also added small holes to allow a loop of 3mm filament to be attached by a cable tie to make a flexible wire support.
// 26 May 2011
...
// Generate nut barrier with conical lead in under.  Replace this...

	//translate(v = [10, 0, -31]) cylinder(h = 4, r=m8_nut_diameter/2+thin_wall, $fn=6, center=true);

// ... with this--------
	difference()
	{
	translate(v = [0, 0, -29]) cylinder(h = 16, r=m8_nut_diameter/2+thin_wall, $fn=6, center=true);
	translate(v = [0, 0, -32]) cylinder(h = 10, r1=10+4, r2=4, $fn=30, center=true);
	cylinder(h = 100, r=4.5, $fn=30, center=true);
	}
...
// Cable support holes
translate([30, -4, 48]) rotate([0,90,0])cylinder(h = 20, r=1.5, $fn=15, center=true);
translate([15, -4, 45]) rotate([90,0,0])cylinder(h = 20, r=1.5, $fn=15, center=true);
}
}
xendmotor();
